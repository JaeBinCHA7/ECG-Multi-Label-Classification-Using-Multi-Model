...
def visualize_pair_results(epoch, real, fake):
    save_dir = './figure/'

    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    save_plot_pair_sample(real, fake, epoch, num_epochs=100,
                          impath=os.path.join(save_dir, 'epoch%03d' % epoch + '.png'))


def visualize_pair_results_12leads(epoch, real, fake):
    save_dir = './figure/'

    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    save_plot_pair_sample_12_lead(real, fake, epoch, num_epochs=100,
                                  impath=os.path.join(save_dir, 'epoch%03d' % epoch + '.png'))
...
def save_plot_pair_sample(samples1, samples2, idx, n_samples=6, num_epochs=None, impath=None, ncol=2):
...
def save_plot_pair_sample_12_lead(real, fake, idx, n_samples=6, num_epochs=None, impath=None, ncol=2):
    assert n_samples <= real.shape[2]
    assert n_samples % ncol == 0
    sample_length = real.shape[3]  # N,C,L
