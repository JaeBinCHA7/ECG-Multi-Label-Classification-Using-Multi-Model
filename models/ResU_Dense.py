...
class Bottleneck(nn.Module):
    def __init__(self, nChannels, growthRate, group):
...
class Transition(nn.Module):
    def __init__(self, nChannels, nOutChannels, group, down=False):
...
class ResidualUBlock(nn.Module):
    def __init__(self, out_ch, mid_ch, group, layers, downsampling=True):
...
def _make_dense(nChannels, growthRate, nDenseBlocks, group, bottleneck):
    layers = []
    for i in range(int(nDenseBlocks)):
        if bottleneck:
            layers.append(Bottleneck(nChannels, growthRate, group))
        else:
            layers.append(SingleLayer(nChannels, growthRate, group))
        nChannels += growthRate
    return nn.Sequential(*layers)


class ResU_Dense(nn.Module):
    def __init__(self, nOUT, in_ch=12, out_ch=256, mid_ch=64, groups=1):
        ...
        self.rub_0 = ResidualUBlock(out_ch=out_ch, mid_ch=mid_ch, group=groups, layers=6)
        self.rub_1 = ResidualUBlock(out_ch=out_ch, mid_ch=mid_ch, group=groups, layers=5)
        self.rub_2 = ResidualUBlock(out_ch=out_ch, mid_ch=mid_ch, group=groups, layers=4)
        self.rub_3 = ResidualUBlock(out_ch=out_ch, mid_ch=mid_ch, group=groups, layers=3)
        ...
        self.dense1 = _make_dense(nChannels, growthRate=12, nDenseBlocks=nDenseBlocks, group=groups, bottleneck=True)
        nChannels += nDenseBlocks * growthRate
        nOutChannels = int(math.floor(nChannels * reduction))
        self.trans1 = Transition(nChannels, nOutChannels, group=groups)

        nChannels = nOutChannels
        self.dense2 = _make_dense(nChannels, growthRate=12, nDenseBlocks=nDenseBlocks, group=groups, bottleneck=True)
        nChannels += nDenseBlocks * growthRate
        self.trans2 = Transition(nChannels, out_ch, group=groups)
